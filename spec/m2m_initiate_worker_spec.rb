...
params = {
  ...
  depositor:, collection:, ingest_manifest: '',
...
RSpec.describe 'M2MInitiateWorker' do
  ...
  let(:m2m_initiate_worker) do
    ...
    allow(s3_manager).to receive(:upload_file)
      .with("#{depositor}/#{collection}/1/one.txt", anything).and_return(true)
    allow(s3_manager).to receive(:upload_file)
      .with("#{depositor}/#{collection}/2/two.txt", anything).and_return(true)
    ...
    queuer = spy('queuer')
    allow(queuer).to receive(:queue_ingest).with(any_args).and_return(true)
    named_params = {
      s3_manager:,
      package_zip_dir:,
      package_extract_dir:,
  ...
  after do
    ...
    FileUtils.rm_f(package_extract_cleanup_dir)

    temp_manifest_dir = File.join(ingest_root, zip_filename)
    FileUtils.rm_f(temp_manifest_dir)

    FileUtils.rm_f(temp_extracted_dir)
  ...
  context 'when generating ingest manifest from package' do
    xit 'creates manifest from extracted package content' do
      ...
      manifest_file = handler.create_ingest_manifest_file(msg: m2m_msg, manifest:)
